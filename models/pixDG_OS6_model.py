...
class PixDGOS6Model(BaseModel):
    ...
    def forward(self):
        ...
        self.fake_SBH, self.fake_SB = self.netG(torch.cat([self.real_SA, self.real_SAH], 1))
        ...
        self.fake_SBH_idt, self.fake_SB_idt = self.netG(torch.cat([self.real_SB, self.real_SBH], 1))  # G(A)
    ...
    def test(self):
        ...
        with torch.no_grad():
            # 为了可视化，不会对网络进行操作
            self.fake_TBH, self.fake_TB = self.netG(torch.cat([self.real_TA, self.real_TAH], 1))
