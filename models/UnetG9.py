...
class UnetG9(nn.Module):
    """Create a Unet-based generator"""

    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        super(UnetG9, self).__init__()
        self.hfc_filter = HFCFilter(21, 20)
        self.hfc_pool = nn.AvgPool2d(2)
        unet_block9 = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, norm_layer=norm_layer, innermost=True)  # add the innermost layer
        ...
        unet_block3 = UnetSkipConnectionBlock(ngf * 2, ngf * 4, input_nc=ngf * 2 + 3, norm_layer=norm_layer)
        unet_block2 = UnetSkipConnectionBlock(ngf, ngf * 2, input_nc=ngf+3, norm_layer=norm_layer)
        unet_block1 = UnetSkipConnectionBlock(output_nc, ngf, input_nc=6, outermost=True, norm_layer=norm_layer)  # add the outermost layer
    ...
    def forward(self, x, mask):
        ...
        d2 = self.down2(torch.cat([d1, hfc0], 1))
        # d2 = self.down2(d1)
        hfc1 = self.hfc_pool(hfc0)
        d3 = self.down3(torch.cat([d2, hfc1], 1))
        d4 = self.down4(d3)
        d5 = self.down5(d4)
        d6 = self.down6(d5)
        d7 = self.down7(d6)
        ...
        out_hfc = self.hfc_filter(final_out, mask)

        return final_out, hfc0, out_hfc
...
if __name__ == '__main__':
    ...
    net = UnetG(input_nc=3,output_nc=3,use_dropout=True)
    net.to('cuda:0')
