...
class PixDGOS3KernelAblationTMIModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_L1_cyc', type=float, default=100.0)
            # parser.add_argument('--lambda_L1_idt', type=float, default=5.0)
            # parser.add_argument('--lambda_L1H_idt', type=float, default=5.0)
        parser.add_argument('--use_HFC', action='store_true')
        parser.add_argument('--use_L1H', action='store_true')
        parser.add_argument('--use_L1_cyc', action='store_true')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_L1H', 'G_L1', 'G_L1_cyc', 'G']
        ...
        self.use_HFC = opt.use_HFC
        self.use_L1H = opt.use_L1H
        self.use_L1_cyc = opt.use_L1_cyc
    ...
    def set_input(self, input, isTrain=None):
        ...
        if not self.isTrain or isTrain is not None:
            ...
            if self.use_HFC:
                ...
                self.real_TAH = mul_mask(self.real_TA, self.T_mask)
            ...
            if self.use_HFC:
                ...
                self.real_SAH = mul_mask(self.real_SA, self.S_mask)
                self.real_SBH = mul_mask(self.real_SB, self.S_mask)

    def forward(self):
        """Run forward pass; called by both functions <optimize_parameters> and <test_total>."""

        if self.use_HFC and (self.use_L1H or self.use_L1_cyc):
        ...
        elif self.use_HFC and not (self.use_L1H or self.use_L1_cyc):
            self.fake_SB = self.netG(self.real_SAH)
            self.fake_SBH = self.fake_SB
            self.fake_SBH = mul_mask(self.fake_SBH, self.S_mask)
            self.fake_SB_HFC = hfc_mul_mask_list(self.hfc_filter_list, self.fake_SB, self.S_mask)
        else:
            ...
            self.fake_SBH = self.fake_SB
            self.fake_SBH = mul_mask(self.fake_SBH, self.S_mask)
            self.fake_SB_HFC = self.fake_SBH
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if self.use_HFC and (self.use_L1H or self.use_L1_cyc):
                ...
                self.fake_TB = self.netG(self.real_TAH)
                self.fake_TBH = self.fake_TB
            else:
            ...
            if self.use_HFC:
                ...
                self.fake_TB_HFC = mul_mask(self.fake_TB, self.T_mask)
    ...
    def backward_G(self):
        ...
        # ！！！Lcyc
        if self.use_L1_cyc:
            self.loss_G_L1_cyc = self.criterionL1(self.fake_SB_HFC, self.fake_SBH.detach()) * self.opt.lambda_L1_cyc
        else:
            self.loss_G_L1_cyc = 0

        self.loss_G = self.loss_G_L1 + self.loss_G_L1H + self.loss_G_L1_cyc
