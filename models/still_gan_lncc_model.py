...
from models.backbone.still_gan_backbone import LuminanceLoss, StructureLoss, NCCLoss


def mul_mask(image, mask):
    return (image + 1) * mask - 1

class StillGANLnccModel(BaseModel):
    """
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B','NClcc_A','NClcc_B']
        ...
        if self.isTrain:
            ...
            self.criterionNCCL= NCCLoss().to(self.device)
    ...
    def backward_G(self):
        ...
        self.loss_NClcc_A= self.criterionNCCL(self.real_A, self.fake_A) * 2
        self.loss_NClcc_B = self.criterionNCCL(self.real_B, self.fake_B) * 2
        ...
        + self.loss_NClcc_A + self.loss_NClcc_B
