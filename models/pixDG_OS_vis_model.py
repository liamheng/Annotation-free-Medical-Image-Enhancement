# -*- coding: UTF-8 -*-
"""
@Function:从two-stage到one-stage，建立简化模型，可以看到L1的损失的图
...
class PixDGOSVisModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_L1H', 'G_L1', 'G_L1_HFC', 'G_L1H_idt', 'G_L1_idt', 'G']

        self.visual_names_train = ['real_SA', 'real_SAH', 'fake_SBH', 'fake_SB',
                                   'real_SB', 'real_SBH', 'fake_SBH_idt', 'fake_SB_idt',
                                   'L1_vis', 'L1H_vis', 'L1_HFC_vis']
    ...
    def backward_G(self):
        ...
        self.loss_G_L1_idt = self.criterionL1(self.fake_SB_idt, self.real_SB) * self.opt.lambda_L1H_idt
        self.loss_G_L1H_idt = self.criterionL1(self.fake_SBH_idt, self.real_SBH) * self.opt.lambda_L1H_idt
        self.loss_G = self.loss_G_L1 + self.loss_G_L1H + self.loss_G_L1_HFC + self.loss_G_L1_idt + self.loss_G_L1H_idt

        self.loss_G.backward()

    def vis_loss(self):
        self.L1_vis = torch.abs(self.fake_SB - self.real_SB) - 1
        self.L1H_vis = torch.abs(self.fake_SBH - self.real_SBH) - 1
        self.L1_HFC_vis = torch.abs(self.fake_SB_HFC - self.real_SBH) - 1
