...
class StillGANSCRModel(BaseModel):
    """
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B','cycle_sobel_A','cycle_sobel_B',
                           'D_A_sobel', 'G_A_sobel', 'idt_A_sobel', 'D_B_sobel', 'G_B_sobel', 'idt_B_sobel']
        # specify the images you want to save/display. The training/test_total scripts will call <BaseModel.get_current_visuals>
        visual_names_A = ['real_A', 'fake_B', 'rec_A','real_A_sobel','fake_B_sobel','rec_A_sobel']
        visual_names_B = ['real_B', 'fake_A', 'rec_B','real_B_sobel','fake_A_sobel','rec_B_sobel']
        if self.isTrain and self.opt.lambda_identity > 0.0:  # if identity loss is used, we also visualize idt_B=G_A(B) ad idt_A=G_A(B)
            visual_names_A.append('idt_B')
            visual_names_B.append('idt_A')
            visual_names_A.append('idt_B_sobel')
            visual_names_B.append('idt_A_sobel')
        ...
        if self.isTrain:
            self.model_names = ['G_A', 'G_B', 'D_A', 'D_B','D_A_sobel','D_B_sobel']
        ...
        if self.isTrain:  # define discriminators
            ...
            self.netD_B = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD_A_sobel = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD_B_sobel = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
        ...
        if self.isTrain:
            ...
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters(),self.netD_A_sobel.parameters(),self.netD_B_sobel.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            self.loss_idt_A_sobel = self.criterionIdt(self.idt_A_sobel, self.real_B_sobel) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed: ||G_B(A) - A||
            self.idt_B,self.idt_B_sobel = self.netG_B(self.real_A)
            self.loss_idt_B = self.criterionIdt(self.idt_B, self.real_A) * lambda_A * lambda_idt
            self.loss_idt_B_sobel = self.criterionIdt(self.idt_B_sobel, self.real_A_sobel) * lambda_B * lambda_idt
        ...
        self.loss_G_A_sobel = self.criterionGAN(self.netD_A_sobel(self.fake_B_sobel), True)
        # GAN loss D_B(G_B(B))
        self.loss_G_B = self.criterionGAN(self.netD_B(self.fake_A), True)
        self.loss_G_B_sobel= self.criterionGAN(self.netD_B_sobel(self.fake_A_sobel), True)
        # Forward cycle loss || G_B(G_A(A)) - A||
        self.loss_cycle_A = self.criterionCycle(self.rec_A, self.real_A) * lambda_A
        # Backward cycle loss || G_A(G_B(B)) - B||
        self.loss_cycle_B = self.criterionCycle(self.rec_B, self.real_B) * lambda_B

        self.loss_cycle_sobel_A = self.criterionCycle(self.rec_A_sobel, self.real_A_sobel) * lambda_A
        # Backward cycle loss || G_A(G_B(B)) - B||
        self.loss_cycle_sobel_B = self.criterionCycle(self.rec_B_sobel, self.real_B_sobel) * lambda_B
        ...
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A \
                    ...
                    + self.loss_cycle_sobel_A + self.loss_cycle_sobel_B + self.loss_idt_A_sobel + self.loss_idt_B_sobel \
                    + self.loss_G_A_sobel + self.loss_G_B_sobel
    ...
    def optimize_parameters(self):
        ...
        # G_A and G_B
        self.set_requires_grad([self.netD_A, self.netD_B,self.netD_B_sobel,self.netD_A_sobel], False)  # Ds require no gradients when optimizing Gs
        self.optimizer_G.zero_grad()  # set G_A and G_B's gradients to zero
        self.backward_G()             # calculate gradients for G_A and G_B
        self.optimizer_G.step()       # update G_A and G_B's weights
        # D_A and D_B
        self.set_requires_grad([self.netD_A, self.netD_B,self.netD_B_sobel,self.netD_A_sobel], True)
        self.optimizer_D.zero_grad()   # set D_A and D_B's gradients to zero
        self.backward_D_A()      # calculate gradients for D_A
        self.backward_D_B()      # calculate graidents for D_B
        self.backward_D_A_sobel()  # calculate gradients for D_A
        self.backward_D_B_sobel()  # calculate graidents for D_B
