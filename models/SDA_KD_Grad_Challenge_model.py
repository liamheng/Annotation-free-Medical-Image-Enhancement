...
class SDAKDgradChallengeModel(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        self.visual_names_train = ['target_gt', 's_fake','target_noise',
                                   't_fake']
        # self.visual_names_test = ['target_gt', 's_fake', 'target_noise',
        #  't_fake']
    ...
    def forward(self):
        ...
        self.s_fake = self.netS(self.target_noise)  # generate output image given the input_color data_A
        self.t_fake = self.netT(self.target_noise)
    ...
    #     self.visual_names = self.visual_names_test
    #
    #     scales = [1.0, 2.0,3.0]  # Different scales for inference
    #     predictions = []
    #     for scale in scales:
    #         scaled_image = F.interpolate(self.target_noise, scale_factor=scale, mode='bilinear', align_corners=False)
    #
    #         with torch.no_grad():
    #             prediction = self.netS(scaled_image)
    #
    #         predictions.append(
    #             F.interpolate(prediction, size=self.target_noise.shape[2:], mode='bilinear', align_corners=False))
    #
    #         # Calculate the average prediction over different scales
    #
    #     self.s_fake = torch.mean(nn.Tanh()(torch.stack(predictions)), dim=0)
    #     # self.s_fake = torch.mean(torch.stack(predictions), dim=0)
    ...
    def backward(self):
        ...
        self.loss_G_seg = self.criterionContent(self.s_pesudo, self.t_pesudo) * self.opt.lambda_l1
    ...
    def load_networks2(self):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                # TODO : 修改路径
                # load_path = os.path.join("./pre_trained_model/stillgan200", load_filename)

                load_path = os.path.join("./pre_trained_model/train_ultrasound_stillgan_twolow", load_filename)
                net = getattr(self, 'net' + name)
