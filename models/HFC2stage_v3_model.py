...
class HFC2StageV3Model(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netGL = networks.define_G(6, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def forward(self):
        ...
        # 反向传播不会对S1有影响
        self.fake_SB = self.netGL(torch.cat([self.real_SA, self.fake_S1_SH.detach()], dim=1))  # G(A)
        ...
        # 反向传播不会对S1有影响
        self.fake_TB = self.netGL(torch.cat([self.real_TA, self.fake_S1_TH.detach()], dim=1))  # G(A)
    ...
    def backward_GL(self):
        self.loss_G_S2_L1 = self.criterionL1(self.fake_SB, self.real_SB) * self.opt.lambda_S2_L1
        self.loss_G_S2_L1H = self.criterionL1(self.fake_SBH, self.real_SBH) * self.opt.lambda_S2_L1H
