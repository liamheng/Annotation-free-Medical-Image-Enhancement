...
class UnetCombineCAGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        """
        super(UnetCombineCAGenerator, self).__init__()
        assert num_downs == 8
    ...
    def forward(self, x):
        ...
        u8 = self.h_up8(d8)
        u8 = self.cbam8(u8)
        u7 = self.h_up7(torch.cat([h_u8, u8, d7], 1))
        u7 = self.cbam7(u7)
        u6 = self.h_up6(torch.cat([h_u7, u7, d6], 1))
        u6 = self.cbam6(u6)
        u5 = self.h_up5(torch.cat([h_u6, u6, d5], 1))
        u5 = self.cbam5(u5)
        u4 = self.h_up4(torch.cat([h_u5, u5, d4], 1))
        u4 = self.cbam4(u4)
        u3 = self.h_up3(torch.cat([h_u4, u4, d3], 1))
        u3 = self.cbam3(u3)
        u2 = self.h_up2(torch.cat([h_u3, u3, d2], 1))
        u2 = self.cbam2(u2)
        u1 = self.h_up1(torch.cat([h_u2, u2, d1], 1))
        # return u1
        return h_u1, u1
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if outermost:
            upconv = nn.ConvTranspose2d(inner_nc * 3, outer_nc,
                                        ...
                                        padding=1)
            cbam = CBAM(outer_nc, reduction_ratio=reduction_ratio)
