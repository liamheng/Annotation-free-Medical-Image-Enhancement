...
class UnetCombine2LayerGenerator(nn.Module):
    ...
    def forward(self, x): # ([2, 3, 256, 256])
        """Standard forward"""
        # downsample
        d1 = self.down1(x)  # ([2, 64, 128, 128])
        d2 = self.down2(d1) # ([2, 128, 64, 64])
        d3 = self.down3(d2) # ([2, 256, 32, 32])
        d4 = self.down4(d3) # ([2, 512, 16, 16])
        d5 = self.down5(d4) # ([2, 512, 8, 8])
        d6 = self.down6(d5) # ([2, 512, 4, 4])
        d7 = self.down7(d6) # ([2, 512, 2, 2])
        d8 = self.down8(d7) # ([2, 512, 1, 1])

        # upsample
        h_u8 = self.up8(d8)                        # ([2, 512, 2, 2])
        h_u7 = self.up7(torch.cat([h_u8, d7], 1))  # ([2, 512, 4, 4])
        h_u6 = self.up6(torch.cat([h_u7, d6], 1))  # ([2, 512, 8, 8])
        h_u5 = self.up5(torch.cat([h_u6, d5], 1))  # ([2, 512, 16, 16])
        h_u4 = self.up4(torch.cat([h_u5, d4], 1))  # ([2, 256, 32, 32])
        h_u3 = self.up3(torch.cat([h_u4, d3], 1))  # ([2, 128, 64, 64])
        h_u2 = self.up2(torch.cat([h_u3, d2], 1))  # ([2, 64, 128, 128])
        h_u1 = self.up1(torch.cat([h_u2, d1], 1))  # ([2, 3, 256, 256])
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if outermost:
            upconv = nn.ConvTranspose2d(inner_nc * 2, outer_nc,
                                        ...
                                        padding=1)
            # 注意：仅仅修改了这个
            h_upconv = nn.ConvTranspose2d(inner_nc * 2, input_nc,
                                        kernel_size=4, stride=2,
                                        padding=1)
            down = [downconv]
        ...
        # *args保存多余变量，保存方式为元组。 **args保存带有变量名的多余变量，保存方式为字典。
...
if __name__ == '__main__':
    ...
    net = UnetCombine2LayerGenerator(3,3,8)
