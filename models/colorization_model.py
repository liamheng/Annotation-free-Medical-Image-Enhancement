...
class ColorizationModel(Pix2PixModel):
    """This is a subclass of Pix2PixModel for images colorization (black & white images -> colorful images).
    ...
    def __init__(self, opt):
        ...
        For visualization, we set 'visual_names' as 'real_A' (input real images),
        'real_B_rgb' (ground truth RGB images), and 'fake_B_rgb' (predicted RGB images)
        We convert the Lab images 'real_B' (inherited from Pix2pixModel) to a RGB images 'real_B_rgb'.
        we convert the Lab images 'fake_B' (inherited from Pix2pixModel) to a RGB images 'fake_B_rgb'.
        """
    ...
    def lab2rgb(self, L, AB):
        """Convert an Lab tensor images to a RGB numpy output
        Parameters:
            L  (1-channel tensor array): L channel images (range: [-1, 1], torch tensor array)
            AB (2-channel tensor array):  ab channel images (range: [-1, 1], torch tensor array)

        Returns:
            rgb (RGB numpy images): rgb output images  (range: [0, 255], numpy array)
        """
        AB2 = AB * 110.0
        L2 = (L + 1.0) * 50.0
