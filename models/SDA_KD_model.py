...
class SDAKDModel(BaseModel):
    ...
    def set_input(self, input, isTrain=None,model=None):
        ...
        if self.is_fiq:
            self.segmentation_mask = input['input_mask'].to(self.device)
        else:
    ...
    def forward(self):
        ...
        if self.isTrain:
            ...
            input_tensor_uint8 = ((self.s_fake + 1) * 255/2).type(torch.uint8)
            # 将Tensor转换为NumPy数组并重新排列轴 (8, 256, 256, 3)
            input_np = input_tensor_uint8.cpu().detach().numpy().transpose(0, 2, 3, 1)

            # 初始化Lab和HSV空间的输出数组 需要变成tensor的储存模式
            output_lab = np.zeros((input_np.shape[0],input_np.shape[3],input_np.shape[1],input_np.shape[2])) # (8,3, 256, 256)

            output_hsv = np.zeros((input_np.shape[0],input_np.shape[3],input_np.shape[1],input_np.shape[2]))
            output_rgb = np.zeros((input_np.shape[0], input_np.shape[3], input_np.shape[1], input_np.shape[2]))
            ...
            for i in range(input_np.shape[0]):
                ...
                image = Image.fromarray(input_np[i]).convert("RGB")
                # # 保存image图像
                # image.save("test_cat"+str(i)+".png")
                image_hsv = image.convert("HSV")
                image_lab = ImageCms.applyTransform(image, self.rgb2lab_transform)
                image_rgb = np.asarray(image).astype('float32')
                output_rgb [i] =  transformList(image_rgb)

                image_hsv = np.asarray(image_hsv).astype('float32')
                output_lab[i] = transformList(image_hsv)


                image_lab  = np.asarray(image_lab).astype('float32')
                output_hsv[i] = transformList(image_lab)



            output_lab = torch.FloatTensor(output_lab).to(self.device)
            output_hsv = torch.FloatTensor(output_hsv).to(self.device)
            output_rgb = torch.FloatTensor(output_rgb).to(self.device)

            # x,y,z,h,quality = self.mcf(self.s_fake, output_hsv,output_lab)
            x, y, z, h, quality = self.mcf(output_rgb, output_hsv, output_lab)
            ...
            for i in range(quality.shape[0]):
                # 标签为接受并且概率大于0.8
                # TODO
               # if (quality[i][0] > 0.8) & (torch.argmax(quality[i]) == 0):
       ...
       # TODO : 实验看看这步有啥用
        self.s_fake_seg = self.s_fake_seg * self.mask
        self.t_fake_seg = self.t_fake_seg * self.mask
        # 二值化操作
        # self.fake_seg[self.fake_seg >= 0] = 1
        # self.fake_seg[self.fake_seg < 0] = 0
        # self.fake_seg = torch.where(self.fake_seg >= 0, torch.tensor(1, device=self.fake_seg.device,requires_grad=True),
        #                             torch.tensor(0, device=self.fake_seg.device,requires_grad=True)).float()
    ...
    def load_networks2(self):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                # TODO : 修改路径
                load_path = os.path.join("./pretrain/04_17_newDrive_unet_combine_2layer_L1_5_dropout_withTanh_lamda=0.95_bs=4_addbias", load_filename)
                net = getattr(self, 'net' + name)
