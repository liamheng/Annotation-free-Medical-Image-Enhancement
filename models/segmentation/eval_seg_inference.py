...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_root_dir', type=str, default=r'D:\research\cataract\public2+public2_100_cataract\select_journal')
    # parser.add_argument('--data_dir', type=str, default='/images/liuhaofeng/Project/pixDRP/datasets/fiq_1229/target_image_multi_seg')
    parser.add_argument('--mask_dir', type=str, default=r'D:\Project\pixDA\pixDA_GM\datasets\TMI_public_1129\target_mask')
    ...
    for data_dir in dir_list:
        for seg_model in ['seg_s2.pth', 'seg_s3.pth']:

            full_data_dir = os.path.join(data_root_dir, data_dir)
            seg_output_dir = full_data_dir

            # seg_model = 'seg_s3.pth'
            inference_loader, model = eval_seg_init(seg_model, full_data_dir, args.mask_dir, '', '',
                                                    False, False, device)
            seg_dir = os.path.join(seg_output_dir, seg_model.split('.')[0])
            print('saving the segmentation result in {}'.format(seg_dir))
            metrics = eval_seg_run(model, full_data_dir, device, output_dir=seg_dir, need_mask=args.need_mask)
