...
class SegInferenceDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask_name = image_name.split('.')[0].split('-')[0] + '.png'
        image_path = os.path.join(self.data_dir, image_name)
        # TODO:mask的路径修改这里
        mask_path = os.path.join(self.mask_dir, mask_name)
        # mask_path = r'D:\research\high_low_quality\non_reference\select\visualization\127_left\127_left.png'
        ...
        mask[mask < th] = 0.0
        if self.gt_dir != '':
            gt_path = os.path.join(self.gt_dir, mask_name.split('.')[0]+'_fake_B.png')
            gt_mask_path = os.path.join(self.gt_mask_dir, mask_name)
