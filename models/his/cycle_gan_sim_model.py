...
class CycleGANSimModel(BaseModel):
    """
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        """
        parser.set_defaults(no_dropout=True, source_size_count=1, netG='unet_256',
                            load_size=286, crop_size=256)  # default CycleGAN did not use dropout
        if is_train:
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A', 'D_B', 'G_B', 'cycle_B', 'idt_B', 'G_BV_OD']
    ...
    def set_input(self, input, isTrain=True):
        ...
        # if self.isTrain:
            ...
            self.real_A = input['SA'].to(self.device)
            self.real_B = input['TB'].to(self.device)
            self.A_mask = input['S_mask'].to(self.device)
            self.image_paths = input['SA_path']
        else:
            self.real_A = input['SA'].to(self.device)
            self.real_B = input['SA'].to(self.device)
            self.A_mask = input['S_mask'].to(self.device)
            self.image_paths = input['SA_path']
        # AtoB = self.opt.direction == 'AtoB'
    ...
    def backward_G(self):
        ...
        lambda_G_BV_OD = self.opt.lambda_G_BV_OD
        # Identity loss
        if lambda_idt > 0:
        ...
        self.loss_G_BV_OD = self.criterionCycle(self.rec_A * self.A_mask, self.real_A * self.A_mask) * lambda_G_BV_OD
        # Backward cycle loss || G_A(G_B(B)) - B||
        self.loss_cycle_B = self.criterionCycle(self.rec_B, self.real_B) * lambda_B
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.loss_G_BV_OD
