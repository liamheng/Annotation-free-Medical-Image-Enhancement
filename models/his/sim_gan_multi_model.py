...
class SimGANMultiModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_G_BV_OD', type=float, default=10.0, help='weight for L1 loss')
            parser.add_argument('--lambda_G', type=float, default=1, help='weight for G loss')
            parser.add_argument('--lambda_G_D', type=float, default=1, help='weight for G loss')
            # parser.add_argument('--lambda_G_multi', type=float, default=100.0, help='weight for G loss')
    ...
    def forward(self):
        ...
        # self.fake_SB, self.real_SA_feature = self.netG(self.real_SA)  # G(A)
        # _, self.fake_SB_feature = self.netG(self.fake_SB)  # G(A)
    ...
    def backward_D(self):
        ...
        pred_real = self.netD(self.real_TB.detach())
        # D希望假就是假，真就是真
    ...
    def backward_G(self):
        ...
        # Second, G(A) = B
        self.loss_L1 = self.criterionL1(self.fake_SB, self.real_SA) * self.opt.lambda_L1
        self.loss_G_BV_OD = self.criterionL1(self.fake_SB * self.S_mask, self.real_SA * self.S_mask) * self.opt.lambda_G_BV_OD
        # self.loss_G_multi = 0
        # for (SA_f, SB_f, r) in zip(self.real_SA_feature, self.fake_SB_feature, self.w_ratio):
        #     self.loss_G_multi += r * self.criterionL1(SA_f, SB_f)
        # # combine loss and calculate gradients
        # self.loss_G_multi = self.opt.lambda_G_multi * self.loss_G_multi
        # TODO:更正这个权重？
        # self.loss_G = self.loss_G_GAN + self.loss_G_BV_OD + self.loss_G_multi + self.loss_L1
        self.loss_G = self.loss_G_GAN + self.loss_G_BV_OD + self.loss_L1
