...
class PixDAGTMIDDPFixModel(BaseModel):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['DP', 'DP_fake', 'DP_real',
                           ...
                           'G_DP', 'G_L1', 'G_LG', 'G_DDP', 'G']

        self.visual_names_train = ['real_SA', 'real_SAG', 'fake_SB',
                             'fake_SBG', 'real_SB', 'real_SBG',
                             'real_TA', 'real_TAG',
                             'fake_TB', 'fake_TBG']
        self.visual_names_test = ['real_TA', 'real_TAG', 'fake_B', 'fake_TBG']
    ...
    def set_input(self, input, isTrain=None):
        ...
        if isTrain:
            self.real_SA = input['A' if AtoB else 'B'].to(self.device)
            self.real_SB = input['B' if AtoB else 'A'].to(self.device)
            self.real_TA = input['TA' if AtoB else 'TB'].to(self.device)
            self.S_mask = input['A_mask'].to(self.device)
            self.T_mask = input['TA_mask'].to(self.device)
            ...
            # FIQ_inference_dataset
            self.real_TA = input['A' if AtoB else 'B'].to(self.device)
            self.real_B = input['B' if AtoB else 'A'].to(self.device)
            self.T_mask = input['A_mask'].to(self.device)
