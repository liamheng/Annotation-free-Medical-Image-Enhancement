...
class PixDGOS3KernelOriCycTMIModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='instance', netG='unet_combine_2layer', dataset_mode='aligned', no_dropout=True,
                            lr=0.001, output_nc=3)
        if is_train:
            parser.set_defaults(pool_size=0, gan_mode='vanilla')
            parser.add_argument('--lambda_L1', type=float, default=3.0)
            parser.add_argument('--lambda_L1H', type=float, default=10.0)
            parser.add_argument('--lambda_L1_HFC', type=float, default=10.0)
