...
class CataractISecretDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.dir_source_usable = os.path.join(opt.dataroot, 'source_usable')  # get the images directory
        self.dir_source_mask = os.path.join(opt.dataroot, 'source_mask')  # get the images directory
        self.dir_source_usable_mask = os.path.join(opt.dataroot, 'source_usable_mask')  # get the images directory
        ...
        self.source_usable_paths = sorted(make_dataset(self.dir_source_usable, opt.max_dataset_size))  # get images paths
        self.target_paths = sorted(make_dataset(self.dir_target, opt.max_dataset_size))  # get images paths

        self.source_usable_mask_paths = sorted(make_dataset(self.dir_source_usable_mask, opt.max_dataset_size))  # get images paths
    ...
    def __getitem__(self, index):
        ...
        source_usable_path = self.source_usable_paths[index % len(self.source_usable_paths)]
        source_usable_mask_path = os.path.join(self.dir_source_usable_mask, os.path.split(source_usable_path)[-1].replace('jpg', 'png'))
        ...
        SAU = Image.open(source_usable_path).convert('RGB')
        TA = Image.open(target_path).convert('RGB')

        SA_mask = Image.open(source_mask_path).convert('L')
        SB_mask = SA_mask
        SAU_mask = Image.open(source_usable_mask_path).convert('L')
        ...
        source_usable_transform_params = get_params(self.opt, SA.size)
        source_usable_A_transform, source_usable_A_mask_transform = get_transform_six_channel(self.opt, source_usable_transform_params, grayscale=(self.input_nc == 1))
        ...
        SAU = source_usable_A_transform(SAU)
        SAU_mask = source_A_mask_transform(SAU_mask)

        TA = target_A_transform(TA)
        T_mask = target_A_mask_transform(TA_mask)

        return {'SA': SA, 'SB': SB, 'S_mask': S_mask, 'SA_path': source_path, 'SAU': SAU, 'SAU_mask': SAU_mask,
                'SB_path': source_path, 'TA': TA, 'T_mask': T_mask, 'TA_path': target_path, 'SAU_path': source_usable_path}
