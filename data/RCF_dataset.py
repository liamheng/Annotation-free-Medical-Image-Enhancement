...
class RCFDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.input_mask = os.path.join(opt.dataroot, 'mask')  # get the images directory
    ...
    def __getitem__(self, index):
        ...
        # 为了适配target
        if len(os.path.split(image_path)[-1].split('_')) == 1:
            image_name = os.path.split(image_path)[-1].split('_')[0].replace('A.jpg', 'B.jpg')
        else:
            image_name = os.path.split(image_path)[-1].split('_')[0].replace('A', 'B.jpg')
        mask_name = os.path.split(image_path)[-1].split('_')[0].replace('.jpg', '') + '.png'
