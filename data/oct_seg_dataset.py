...
def get_transform_seg_channel(opt, params, method=Image.NEAREST, is_train=True, grayscale=False):
    transform_list = []
    mask_transform_list = []
    ...
    # TODO:对图像进行阈值处理
    # mask_transform_list.append(transforms.Resize(osize, Image.BICUBIC))
...
class OctSegDataset(BaseDataset):
    def __init__(self, opt):
        BaseDataset.__init__(self, opt)
        # for eyeq
        self.is_train = opt.isTrain
        if self.is_train:
            self.image_dir = os.path.join(opt.dataroot, 'remove_bad_image')  # get the images directory
            # self.mask_dir = os.path.join(opt.dataroot, 'source_mask')  # get the images directory
            self.ves_dir = os.path.join(opt.dataroot, 'remove_bad_mask')  # get the images directory
        else:
            # self.image_dir = os.path.join(opt.dataroot, 'target_gt')  # get the images directory
            self.image_dir = os.path.join(opt.dataroot, 'test_300')  # get the images directory
            # self.image_dir = os.path.join(opt.dataroot, 'gt')  # get the images director
            # self.mask_dir = os.path.join(opt.dataroot, 'target_mask')  # get the images directory
            self.ves_dir = os.path.join(opt.dataroot, 'test_300_mask')  # get the images directory

        self.image_paths = sorted(make_dataset(self.image_dir, opt.max_dataset_size))  # get images paths
        # self.mask_paths = sorted(make_dataset(self.ves_dir, opt.max_dataset_size))  # get images paths
    ...
    def __getitem__(self, index):
        ...
        transform_image, transform_mask = get_transform_seg_channel(self.opt, transform_params, method=Image.BICUBIC,
                                                                    is_train=self.is_train, grayscale=self.input_nc == 1)
        image = transform_image(image)
        # mask = transform_mask(mask)
        # TODO：对mask进行处理 将0.1586-0.9412映射到1-6
        if self.is_train:
            ves_path = os.path.join(self.ves_dir, image_name)
            ves = Image.open(ves_path).convert('L')
            ves = transform_mask(ves)
            ves = torch.squeeze(ves, dim=0) # 1, 512, 512 -> 512, 512 去掉多余维度
            label = torch.zeros_like(ves, dtype=torch.long)
            # label_list = []
            # label_list = [torch.tensor()]
            background_layer = ves < self.threshold_list[0] # 背景颜色
            label[background_layer==1] = 0
            # label_list.append(background_layer)
            for i in range(len(self.threshold_list)-1):
                layer_bigger = ves > self.threshold_list[i]
                layer_lower = ves <= self.threshold_list[i+1]
                layer = layer_bigger * layer_lower # 交集  取到每层的标签
                label[layer==1] = i+1    # 从1开始递增
            #     label_list.append(layer)
            ...
            label = torch.ones((256, 256), dtype=torch.long)
        # return {'images': images, 'ves': ves, 'mask': mask, 'image_path': image_path}
        return {'images': image, 'ves': label, 'image_path': image_path}
        #
        # # 测试时必须要有mask
        ...
        #     mask = transform_mask(mask)
        #
        #     return {'images': images, 'mask': mask, 'image_path': image_path}
