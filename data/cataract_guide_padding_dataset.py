...
class CataractGuidePaddingDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # os.path.split 将文件名和路径分割开  即从最后一个‘/’开始分割
        ...
        w, h = SAB.size
        w2 = int(w / 2)   # 512
        # crop(x,y,z,w) x,y起始横 纵坐标 z,w结束横 纵坐标
        # 将图片 切分成两张
        ...
        if self.need_mask:
            SA_mask = Image.open(source_path_mask_path).convert('L')  # 灰度图像
            SB_mask = SA_mask
            ...
            S_mask = T_mask = torch.ones_like(SA[:1, :, :], dtype=torch.float32)
