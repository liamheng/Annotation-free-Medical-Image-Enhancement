...
class LowQualityVesDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.dir_source_seg = os.path.join(opt.dataroot, 'source_ves')  # get the image directory
    ...
    def __getitem__(self, index):
        ...
        th = 0.4
        SA_seg[SA_seg >= th] = 1.0
        SA_seg[SA_seg < th] = 0.0

        TA = target_A_transform(TA)
        TA_mask = target_A_mask_transform(TA_mask)

        return {'SA': SA, 'SB': SB, 'S_mask': SA_mask, 'SA_path': source_path, 'SA_seg': SA_seg,
                'SB_path': source_path, 'TA': TA, 'T_mask': TA_mask, 'TA_path': target_path}
