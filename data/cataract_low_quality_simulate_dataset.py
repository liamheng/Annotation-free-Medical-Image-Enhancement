...
class CataractLowQualitySimulateDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.source_usable_size = len(self.source_usable_paths)
    ...
    def __getitem__(self, index):
        ...
        source_mask_path = os.path.join(self.dir_source_mask, os.path.split(source_path)[-1].split('.')[0]+'.png')

        source_usable_index = random.randint(0, self.source_usable_size - 1) if self.isTrain else index % self.source_usable_size
        source_usable_path = self.source_usable_paths[source_usable_index]

        SAB = Image.open(source_path).convert('RGB')
        cup_mask = Image.open(source_mask_path).convert('RGB')
        segmentation_mask = Image.open(source_segmentation_path).convert('RGB')

        SU = Image.open(source_usable_path).convert('RGB')
        w, h = SAB.size
        w2 = int(w / 2)
