...
def get_mask(img):
    if img.ndim ==3:
        ...
        raise 'images dim is not 1 or 3'
    h,w = g_img.shape
...
def preprocess(img):
    ...
    #   img : origin images
    # return:
    #   result_img: preprocessed images
    #   mask: mask for preprocessed images
...
def preprocess_ves(img, ves):
    ...
    r_img,r_border=remove_back_area(r_img,bbox=bbox)
    mask,_=remove_back_area(mask,border=r_border)
    r_ves,r_ves_border=remove_back_area(ves,bbox=bbox)

    r_img,sup_border=supplemental_black_area(r_img)
    mask,_=supplemental_black_area(mask,border=sup_border)
    r_ves,_=supplemental_black_area(r_ves,border=sup_border)
    return r_img,(mask*255).astype(np.uint8),r_ves

def preprocess_label_list(img, label_list):
    # preprocess images
    #   img : origin images
    # return:
    #   result_img: preprocessed images
    ...
    r_label_list = []
    for l in label_list:
        t, _ = remove_back_area(l, bbox=bbox)
        r_label_list.append(t)

    r_img,sup_border=supplemental_black_area(r_img)
    mask,_=supplemental_black_area(mask,border=sup_border)
    r2_label_list = []
    for l in r_label_list:
        t, _ = supplemental_black_area(l,border=sup_border)
        r2_label_list.append(t)
    return r_img,(mask*255).astype(np.uint8),r2_label_list