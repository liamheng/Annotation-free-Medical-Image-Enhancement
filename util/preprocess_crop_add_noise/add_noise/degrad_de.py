...
def DE_COLOR(img, brightness=0.5, contrast=0.6, saturation=0.6):
    """Randomly change the brightness, contrast and saturation of an images"""
    
    if brightness > 0:
        # brightness_factor = random.uniform(max(0.0, 1.0 - brightness), 1.0 + brightness) # brightness factor
        p = random.uniform(0,1)
        if p > 0.5:
            brightness_factor = random.uniform(1 - brightness, 0.7) # brightness factor
        else:
            brightness_factor = random.uniform(1.2, 1 + brightness) # brightness factor
...
def DE_HALO(img, h, w, brightness_factor, center=None, radius=None):
    '''
    Defined to simulate a 'ringlike' halo noise in fundus images
    ...
    delta_circle = np.array([weight_r[num]*delta_circle])
...
def DE_HOLE(img, h, w, region_mask, center=None, diameter=None):
    ...
    :return:
    '''
    # if radius is None: # use the smallest distance between the center and images walls
    ...
    center =[random.randint(w/4,w*3/4),random.randint(int(h*3/8),int(h*5/8))]
    Y, X = np.ogrid[:h, :w]
...
def DE_SPOT(img, h, w, center=None, radius=None):
    '''
    :param s_num:  The number of the generated artifacts spot on the fundus images
    :param radius: Define the size of each spot
    :param center: Position of each spot on the fundus images
    :param K:      Weight of original fundus images value
    ...
    for i in range(s_num):
        ...
        # if center is None: # in the middle of the images
...
def DE_BLUR(img, h, w, center=None, radius=None,ration=1):
    '''
    ...
    # sigma = 15+(25-15) * random.random() * 0.3
    sigma = 5 + (25 - 15) * random.uniform(0,1) * 1
    rad_w = random.randint(int(sigma/3), int(sigma/2))
    rad_h = random.randint(int(sigma/3), int(sigma/2))
    if (rad_w % 2) == 0: rad_w = rad_w + 1
    if(rad_h % 2) ==0 : rad_h =rad_h + 1

    img1 = cv2.GaussianBlur(img, (rad_w,rad_h), sigma)
    ...
    return img1, blur_params


def DE_process(img, mask, h, w, de_type, de_image_type):
    params = {}
    if de_type == '001':
    ...
    if de_image_type == '01':
        # img = transform(img)
        # img = img.numpy()
        # random_var = random.uniform(0.05, 0.15)
        # img = skimage.util.random_noise(img, mode='speckle', var=random.uniform(0.03, 0.08))
        # img = skimage.util.random_noise(img, mode='s&p', amount=0.1, salt_vs_pepper=0.5)
        img = skimage.util.random_noise(img, mode='gaussian', var=random.uniform(0.01, 0.03))
    elif de_image_type == '10':
        img = skimage.util.random_noise(img, mode='speckle', var=random.uniform(0.02, 0.05))

        # img = skimage.util.random_noise(img, mode='speckle')
