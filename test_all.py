"""General-purpose test_total script for images-to-images translation.

Once you have trained your model with train.py, you can use this script to test_total the model.
...
Example (You need to train models first or download pre-trained models from our website):
    Test a CycleGAN model (both sides):
        python test_total.py --dataroot ./datasets/maps --name maps_cyclegan --model cycle_gan

    Test a CycleGAN model (one side only):
        python test_total.py --dataroot datasets/horse2zebra/testA --name horse2zebra_pretrained --model test_total --no_dropout

    The option '--model test_total' is used for generating CycleGAN results only for one side.
    ...
    Test a pix2pix model:
        python test_total.py --dataroot ./datasets/facades --name facades_pix2pix --model pix2pix --direction BtoA

See options/base_options.py and options/test_options.py for more test_total options.
...
def model_test(testOpt, testDataset, model, web_dir, eval_test=False, only_fake_TB=False):
    webpage = html.HTML(web_dir,
                        'Experiment = %s, Phase = %s, Epoch = %s' % (testOpt.name, testOpt.phase, testOpt.epoch))
    print('saving the result in {}'.format(web_dir))
    if eval_test:
        model.eval()
    start = time.time()
    print('process finish: {}, batch size: {}'.format(len(testDataset), testOpt.batch_size))
    ...
    for i, data in enumerate(testDataset):
        ...
        save_batch_images(webpage, visuals, img_path, aspect_ratio=testOpt.aspect_ratio,
                    width=testOpt.display_winsize, only_fake_TB=only_fake_TB)
    end = time.time()
    print((end-start)/len(testDataset))
    model.train()
...
def set_fiq_opt(opt):
    ...
    fiqTestOpt.seg_gt_dir = '/data/liuhaofeng/Project/pixDRP/datasets/fiq_seg/target_gt'
    ...
    fiqTestOpt.target_gt_dir = 'target_gt'
    fiqTestOpt.seg_gt_mask_dir = '/data/liuhaofeng/Project/pixDRP/datasets/fiq_seg/target_gt_mask'
    fiqTestOpt.seg_mask_dir = '/data/liuhaofeng/Project/pixDRP/datasets/fiq_seg/target_mask'
...
def set_drive_opt(opt):
    ...
    driveTestOpt.seg_gt_dir = '/data/liuhaofeng/Project/pixDRP/datasets/drive_seg/drive_seg_gt'
    driveTestOpt.seg_gt_mask_dir = '/data/liuhaofeng/Project/pixDRP/datasets/drive_seg/drive_seg_gt_mask'
    driveTestOpt.seg_mask_dir = '/data/liuhaofeng/Project/pixDRP/datasets/drive_seg/drive_seg_mask'
...
def set_cataract_opt(opt):
    ...
    cataractTestOpt.seg_gt_dir = '/data/liuhaofeng/Project/pixDA_GM/datasets/cataract_0830/target_seg_s3'
    cataractTestOpt.seg_gt_mask_dir = '/data/liuhaofeng/Project/pixDA_GM/datasets/cataract_0830/target_gt_mask'
    cataractTestOpt.seg_mask_dir = '/data/liuhaofeng/Project/pixDA_GM/datasets/cataract_0830/target_mask'
