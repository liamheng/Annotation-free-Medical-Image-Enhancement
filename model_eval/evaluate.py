...
def model_eval(opt, test_output_dir='test_latest', meters=None, wrap=True, write_res=True):
    ...
    for image_name in image_name_list:
        ...
        # 初始化操作
        count += 1
        image_num = re.findall(r'[0-9]+', image_name)[0] # 匹配所有连续数字
        ...
        elif 'RF' in opt.dataroot:
            gt_image_name = image_num + '.PNG'
        else:
            gt_image_name = image_name.replace("_"+end_word,".png")
...
def gamma_correction(image, gamma):
    ...
    corrected_image = image.pow(1.0 / gamma)
...
def model_eval_ultrasound(opt, test_output_dir='test_latest', meters=None, wrap=True, write_res=True):
    ...
    for image_name in image_name_list :
        ...
        #
            ...
            # gt_image_name = image_name.replace('_s_fake.png', '.png')
            # gt_image_name = image_name.replace('_fake_EyeQ.png', '.png')
            gt_image_name = image_name.replace('_fake_TB.png', '.png')
        ...
        # post processing
        temp_code = image_name.split('_')[0]
        cv2.imwrite(os.path.join(post_output_dir, image_name.replace('_s_fake.png', '.png')),image)
        # cv2.imwrite(os.path.join(post_output_dir, image_name.replace('_fake_TB.png', '.png')), image)
...
def model_eval_ultrasound_gamma(opt, test_output_dir='test_latest', meters=None, wrap=True, write_res=True):
    # 初始化
    image_size = (256, 256)
    # image_size = (opt.crop_size, opt.crop_size)
    print('evaluating in the images size of {}'.format(image_size))
    ...
    for image_name in image_name_list :
        ...
        gray_pil_image = Image.open(image_path).convert('L')  # 将图像转换为灰度模式
        gray_image_tensor = torch.from_numpy(np.array(gray_pil_image)).unsqueeze(0).unsqueeze(0).float() / 255.0

        # 设置伽马值（大于1增强亮度，小于1增强暗部）
        gamma_value = 1.1

        # 进行伽马校正
        corrected_gray_image = gamma_correction(gray_image_tensor, gamma_value)

        # 将张量转换为NumPy数组，以便保存为图像
        corrected_gray_image_np = (corrected_gray_image.squeeze().numpy() * 255).astype(np.uint8)
        corrected_pil_image = Image.fromarray(corrected_gray_image_np, mode='L')

        # 保存伽马矫正后的图像
        corrected_pil_image.save(os.path.join(post_output_dir, image_name.replace('_s_fake.png', '.png')))
