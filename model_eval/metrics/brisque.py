...
from model_eval.metrics.utils import _validate_input, _reduce
from model_eval.metrics.functional import rgb2yiq, gaussian_filter
...
def _ggd_parameters(x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    assert not torch.isclose(sigma, torch.zeros_like(sigma)).all(), \
        'Expected images with non zero variance of pixel values'
